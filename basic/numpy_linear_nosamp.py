...
class Activation:
    ...
    def sigmoid(input: np.array):
        input_sigmoid = 1 / (1 + np.exp(-4*input))
        # return np.random.binomial(1, input_sigmoid)
        return input_sigmoid
    
    @staticmethod
    def dsigmoid(input: np.array):
        input_sigmoid = 1 / (1 + np.exp(-4*input))
        p = 4*input_sigmoid * (1 - input_sigmoid)
        return p
...
def test_eval(images_test, labels, w1, w2, w3, b1, b2, b3):
    ...
    z1 = Activation.sigmoid(y1)
    y2 = np.matmul(z1, w2) + b2
    z2 = Activation.sigmoid(y2)
...
for ep in range(EPOCH):
    ...
    for i in range(0, num_train, BATCH_SIZE):
        ...
        z1 = Activation.sigmoid(y1)
        dzdy1 = Activation.dsigmoid(y1)
        y2 = np.matmul(z1, w2) + b2
        z2 = Activation.sigmoid(y2)
        dzdy2 = Activation.dsigmoid(y2)
        z3 = np.matmul(z2, w3) + b3
    ...
    acc = test_eval(images_test, labels_test, w1, w2, w3, b1, b2, b3)
    print('accuracy: ', acc)
